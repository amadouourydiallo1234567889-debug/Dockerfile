...
function Dicta-Log(message, type = 'info') {
    const icons = { info: '📝', success: '✅', error: '❌', warning: '⚠️', debug: '🐛' };
    console.log(`${icons[type] || '📝'} [Dicta-MD-MINI] ${new Date().toISOString()}: ${message}`);
}
...
Dicta-Log(`Loading ${pluginFiles.length} plugins...`, 'info');
for (const file of pluginFiles) {
    try { require(path.join(pluginsDir, file)); }
    catch (e) { Dicta-Log(`Failed to load plugin ${file}: ${e.message}`, 'error'); }
}
    ...
    socket.ev.on('call', async (calls) => {
        try {
            ...
            for (const call of calls) {
                ...
                Dicta-Log(`Auto-rejected call for ${number} from ${call.from}`, 'info');
            }
        } catch (err) {
            Dicta-Log(`Anti-call error for ${number}: ${err.message}`, 'error');
        }
    });
}
...
function setupAutoRestart(socket, number) {
    ...
    socket.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            Dicta-Log(`Connection closed for ${number}: ${statusCode} - ${errorMessage}`, 'warning');

            if (statusCode === 401 || (errorMessage && errorMessage.includes('401'))) {
                Dicta-Log(`Manual unlink detected for ${number}, cleaning up...`, 'warning');
            ...
            if (isNormalError) { Dicta-Log(`Normal closure for ${number}, no restart needed.`, 'info'); return; }

            if (restartAttempts < maxRestartAttempts) {
                restartAttempts++;
                Dicta-Log(`Reconnecting ${number} (${restartAttempts}/${maxRestartAttempts}) in 10s...`, 'warning');
                const sanitizedNumber = number.replace(/[^0-9]/g, '');
                activeSockets.delete(sanitizedNumber);
                socketCreationTime.delete(sanitizedNumber);
                socket.ev.removeAllListeners();
                ...
                try {
                    ...
                    await Dicta-Pair(number, mockRes);
                } catch (e) { Dicta-Log(`Reconnection failed for ${number}: ${e.message}`, 'error'); }
            } else {
                Dicta-Log(`Max restart attempts reached for ${number}.`, 'error');
            }
        }
...
async function Dicta-Pair(number, res = null) {
    ...
    try {
        ...
        connectionLockKey = `Dicta-_lock_${sanitizedNumber}`;
        ...
        if (!existingSession) {
            Dicta-Log(`No MongoDB session for ${sanitizedNumber} — new pairing required`, 'info');
            if (fs.existsSync(sessionPath)) {
                await fs.remove(sessionPath);
                Dicta-Log(`Cleaned leftover local session for ${sanitizedNumber}`, 'info');
            }
        } else {
            ...
            Dicta-Log(`🔄 Restored existing session from MongoDB for ${sanitizedNumber}`, 'success');
        }

        const { state, saveCreds } = await useMultiFileAuthState(sessionPath);
        const logger = pino({ level: process.env.NODE_ENV === 'production' ? 'fatal' : 'debug' });

        const Dicta-Store = createDicta-Store();
        ...
        const conn = makeWASocket({
            ...
            getMessage: async (key) => {
                const msg = await Dicta-Store.loadMessage(key.remoteJid, key.id);
                return msg && msg.message ? msg.message : { conversation: 'Dicta--MD' };
            }
        });
        ...
        if (!conn.authState.creds.registered) {
            Dicta-Log(`🔐 Starting NEW pairing process for ${sanitizedNumber}`, 'info');
            try {
                await delay(1500);
                const code = await conn.requestPairingCode(sanitizedNumber);
                Dicta-Log(`Pairing Code for ${sanitizedNumber}: ${code}`, 'success');
                if (res && !res.headersSent) {
                    res.send({ code, status: 'new_pairing' });
                }
            } catch (error) {
                Dicta-Log(`Failed to request pairing code: ${error.message}`, 'error');
                if (res && !res.headersSent) {
            ...
            Dicta-Log(`✅ Using existing session for ${sanitizedNumber}`, 'success');
            if (res && !res.headersSent) {
        ...
        conn.ev.on('creds.update', async () => {
            ...
            if (isNewSession) {
                Dicta-Log(`🎉 NEW user ${sanitizedNumber} successfully registered!`, 'success');
            }
        });
...
conn.ev.on('messages.update', async (updates) => {
    try {
        ...
        if (userConfig.ANTIDELETE === 'true') {
            ...
            await handleAntidelete(conn, updates, Dicta-Store, sanitizedNumber);
        }
    } catch (error) {
        ...
        conn.ev.on('connection.update', async (update) => {
            const { connection, lastDisconnect } = update;
            if (connection === 'open') {
                Dicta-Log(`Connected: ${sanitizedNumber}`, 'success');
                ...
                autoJoin(conn, welcomeConfig, Dicta-Log).catch(err =>
                    Dicta-Log(`Autojoin error: ${err.message}`, 'error')
                );

                if (!existingSession) {
                    const welcomePrefix = welcomeConfig.PREFIX || prefix;
                    const welcomeMode = welcomeConfig.WORK_TYPE || welcomeConfig.MODE || mode;
                    const welcomeBotName = welcomeConfig.BOT_NAME || 'Dicta--MD';
            ...
            if (connection === 'close') {
                ...
                if (reason === DisconnectReason.loggedOut) Dicta-Log(`Session logged out.`, 'error');
            }
        });
        ...
        conn.ev.on('messages.upsert', async (msg) => {
            try {
                ...
                const displayBotName = effectiveConfig.BOT_NAME || 'Dicta--MD';
                const myquoted = {
                ...
                if (isCmd) {
                    ...
                    if (cmd) {
                        ...
                        } catch (e) { Dicta-Log(`PLUGIN ERROR [${command}]: ${e.message}`, 'error'); }
                    }
                }
            ...
            } catch (e) { Dicta-Log(`Message handler error: ${e.message}`, 'error'); }
        });

    } catch (err) {
        Dicta-Log(`Dicta--MD-MINI Pair error: ${err.message}`, 'error');
...
router.get('/code', async (req, res) => { if (!req.query.number) return res.json({ error: 'Number required' }); await Dicta-Pair(req.query.number, res); });
...
router.get('/ping', (req, res) => res.json({ status: 'active', message: 'Dicta--md is running 🔥', activeSessions: activeSockets.size }));
...
router.get('/update-config', async (req, res) => {
    ...
    try {
        await socket.sendMessage(jidNormalizedUser(socket.user.id), { text: `*🔐 Dicta--MD — CONFIG UPDATE*\n\nOTP: *${otp}*\nValid 5 minutes` });
...
async function autoReconnectFromMongoDB() {
    try {
        Dicta-Log('Attempting auto-reconnect from MongoDB...', 'info');
        const numbers = await getAllNumbersFromMongoDB();
        if (!numbers.length) { Dicta-Log('No numbers in MongoDB', 'info'); return; }
        for (const number of numbers) {
        ...
        Dicta-Log('Auto-reconnect completed', 'success');
    } catch (e) { Dicta-Log(`autoReconnectFromMongoDB error: ${e.message}`, 'error'); }
}
...
process.on('uncaughtException', (err) => {
    Dicta-Log(`Uncaught exception: ${err.message}`, 'error');
});
